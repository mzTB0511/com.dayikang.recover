...
- (void)viewDidLoad {
    ...
    context[@"showlogview"] = ^() {
        if (self.webViewBlock) {
            self.webViewBlock(_webView,nil);
        }
    };
    ...
    context[@"shareText"] = ^() {
        if (self.webViewShareBlock) {
            self.webViewShareBlock(_webView,nil);
        }
    };
...
- (void)updateToolbarItems {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        self.navigationController.toolbar.tintColor = [UIColor blackColor];//self.navigationController.navigationBar.tintColor;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (![currentURL isEqualToString:@""]) {
        id jsonObj = getObjectFromJSonString(currentURL);
        if ([jsonObj isKindOfClass:[NSDictionary class]]) {
            
            //展示信息后强制跳转到登录页
            [CommonHUD showHudWithMessage:[jsonObj objectForKey:@"message"] delay:1.0f completion:^{
